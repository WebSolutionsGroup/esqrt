/**
 * NetSuite Table Classification Script
 * 
 * Processes the NetSuite tables metadata CSV and creates an intelligent classification
 * based on NetSuite's navigation hierarchy and table characteristics.
 * 
 * Usage:
 * 1. Open browser console on any page
 * 2. Copy and paste this script
 * 3. Run: classifyNetSuiteTables()
 * 4. Select your netsuite_tables_metadata_*.csv file when prompted
 * 5. Download the classified CSV automatically
 */

(function() {
    'use strict';

    // Include the classification engine
     ...
     */
    function classifyNetSuiteTables() {
        console.log('🎯 NetSuite Table Classifier loaded!');
        console.log('📁 Please select your netsuite_tables_metadata_*.csv file...');
        ...
        fileInput.onchange = function(event) {
            ...
            reader.onload = function(e) {
                try {
                    ...
                    console.log(`✅ Parsed ${tableData.length} tables`);
                    console.log('🔍 Classifying tables...');
                    
                    // Classify each table
                    const classifiedData = tableData.map(table => {
                        ...
                        return {
                            ...
                            'Record Family': table['Record Family'],
                            'Is Custom': table['Is Custom']
                        };
                    });
                    ...
                    classifiedData.forEach(item => {
                        const key = `${item['Data Type']} - ${item['Category']}`;
                    ...
                    // Generate CSV
                    const headers = ['Table ID', 'Table Label', 'Data Type', 'Category', 'Confidence', 'Record Family', 'Is Custom'];
                    ...
                    const filename = `netsuite_tables_classified_${timestamp}.csv`;
                    
                    downloadCSV(outputCSV, filename);
                    
                    console.log(`🎉 Classification complete! Downloaded: ${filename}`);
    ...
    window.classifyNetSuiteTables = classifyNetSuiteTables;
    
    console.log('🎯 NetSuite Table Classifier ready!');
    console.log('🚀 Run: classifyNetSuiteTables() to start classification');
